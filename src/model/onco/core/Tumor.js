...
import BodyStructure from '../../shr/core/BodyStructure';

/**
 * Generated class for onco.core.Tumor.
 * @extends BodyStructure
 */
class Tumor extends BodyStructure {
   ...
   * @returns {IsPrimaryTumor} The onco.core.IsPrimaryTumor
   */
  get isPrimaryTumor() {
   ...
   * @param {IsPrimaryTumor} isPrimaryTumor - The onco.core.IsPrimaryTumor
   */
  ...
  /**
   * Deserializes JSON data to an instance of the Tumor class.
   * The JSON must be valid against the Tumor JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {Tumor} An instance of Tumor populated with the JSON data
   */
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('onco.core', 'Tumor');
  ...
  /**
   * Serializes an instance of the Tumor class to a JSON object.
   * The JSON is expected to be valid against the Tumor JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/onco/core/Tumor' };
  ...
  /**
   * Deserializes FHIR JSON data to an instance of the Tumor class.
   * The FHIR must be valid against the Tumor FHIR profile, although this is not validated by the function.
   ...
   * @returns {Tumor} An instance of Tumor populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/onco/core/Tumor', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      ...
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/onco-core-IsPrimaryTumor-extension') {
        inst.isPrimaryTumor = FHIRHelper.createInstanceFromFHIR('onco.core.IsPrimaryTumor', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
      }
    }
