...
/**
 * Generated class for onco.core.TumorDimensions.
 * @extends CodedLaboratoryObservation
 */
class TumorDimensions extends CodedLaboratoryObservation {
  ...
  /**
   * Get the RelatedTumor.
   * @returns {RelatedTumor} The onco.core.RelatedTumor
   */
  get relatedTumor() {
    return this._relatedTumor;
  }

  /**
   * Set the RelatedTumor.
   * This field/value is required.
   * @param {RelatedTumor} relatedTumor - The onco.core.RelatedTumor
   */
  set relatedTumor(relatedTumor) {
    this._relatedTumor = relatedTumor;
  }

  /**
   * Set the RelatedTumor and return 'this' for chaining.
   * This field/value is required.
   * @param {RelatedTumor} relatedTumor - The onco.core.RelatedTumor
   * @returns {TumorDimensions} this.
   */
  withRelatedTumor(relatedTumor) {
    this.relatedTumor = relatedTumor; return this;
  }
  ...
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('onco.core', 'TumorDimensions');
  ...
  /**
   * Serializes an instance of the TumorDimensions class to a JSON object.
   * The JSON is expected to be valid against the TumorDimensions JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/onco/core/TumorDimensions' };
    ...
    if (this.relatedTumor != null) {
      inst['RelatedTumor'] = typeof this.relatedTumor.toJSON === 'function' ? this.relatedTumor.toJSON() : this.relatedTumor;
    }
    return inst;
  }

  /**
   * Deserializes FHIR JSON data to an instance of the TumorDimensions class.
   * The FHIR must be valid against the TumorDimensions FHIR profile, although this is not validated by the function.
   ...
   * @returns {TumorDimensions} An instance of TumorDimensions populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/onco/core/TumorDimensions', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/onco-core-RelatedTumor-extension') {
        inst.relatedTumor = FHIRHelper.createInstanceFromFHIR('onco.core.RelatedTumor', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
      }
    }
    ...
    for (const fhir_component of fhir['component'] || []) {
      ...
      if (fhir_component['code'] != null && fhir_component['code']['coding'] != null && fhir_component['code']['coding'].some(g => g['code'] != null && g['code'] === '33728-7')) {
      }
      if (fhir_component['code'] != null && fhir_component['code']['coding'] != null && fhir_component['code']['coding'].some(g => g['code'] != null && g['code'] === '372300005')) {
      }
      if (fhir_component['code'] != null && fhir_component['code']['coding'] != null && fhir_component['code']['coding'].some(g => g['code'] != null && g['code'] === '372301009')) {
      }
      if (false) {
      }
    }
