...
class TNMPathologicPrognosticStagePanel extends CancerStagePanel {
  ...
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('oncocore', 'TNMPathologicPrognosticStagePanel');
  ...
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/oncocore/TNMPathologicPrognosticStagePanel', 'uri');
    if (fhir['meta'] != null) {
      ...
      if (fhir_related['target'] != null && fhir_related['target']['reference'] != null && FHIRHelper.conformsToProfile(allEntries.find(e => e.fullUrl === fhir_related['target']['reference']), 'http://example.com/fhir/StructureDefinition/oncocore-TNMPathologicStageGroup')) {
        if (fhir_related['target'] != null) {
          ...
          if (!mappedResources[entryId]) {
            const referencedEntry = allEntries.find(e => e.fullUrl === entryId);
            if (referencedEntry) {
              mappedResources[entryId] = FHIRHelper.createInstanceFromFHIR('oncocore.TNMPathologicStageGroup', referencedEntry['resource'], 'undefined', shrId, allEntries, mappedResources, referencesOut);
            }
          }
