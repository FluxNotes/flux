...
/**
 * Generated class for shr.sex.GenderIdentity.
 * @extends QuestionAnswer
 */
class GenderIdentity extends QuestionAnswer {
  ...
  /**
   * Deserializes JSON data to an instance of the GenderIdentity class.
   * The JSON must be valid against the GenderIdentity JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {GenderIdentity} An instance of GenderIdentity populated with the JSON data
   */
  static fromJSON(json = {}) {
    const inst = new GenderIdentity();
    setPropertiesFromJSON(inst, json);
    return inst;
  }
  /**
   * Serializes an instance of the GenderIdentity class to a JSON object.
   * The JSON is expected to be valid against the GenderIdentity JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/sex/GenderIdentity' };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the GenderIdentity class to a FHIR object.
   * The FHIR is expected to be valid against the GenderIdentity FHIR profile, but no validation checks are performed.
