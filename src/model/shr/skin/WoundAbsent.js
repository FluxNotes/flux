...
class WoundAbsent extends ConditionAbsent {
  ...
  /**
   * Serializes an instance of the WoundAbsent class to a JSON object.
   * The JSON is expected to be valid against the WoundAbsent JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/skin/WoundAbsent' };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the WoundAbsent class to a FHIR object.
   * The FHIR is expected to be valid against the WoundAbsent FHIR profile, but no validation checks are performed.
