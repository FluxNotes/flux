...
/**
 * Generated class for shr.allergy.NoKnownDrugAllergy.
 * @extends NoAdverseSensitivityToSubstance
 */
class NoKnownDrugAllergy extends NoAdverseSensitivityToSubstance {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {NoKnownDrugAllergy} this.
   */
  ...
  /**
   * Deserializes JSON data to an instance of the NoKnownDrugAllergy class.
   * The JSON must be valid against the NoKnownDrugAllergy JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {NoKnownDrugAllergy} An instance of NoKnownDrugAllergy populated with the JSON data
   */
  static fromJSON(json = {}) {
    const inst = new NoKnownDrugAllergy();
    setPropertiesFromJSON(inst, json);
    return inst;
  }
  /**
   * Serializes an instance of the NoKnownDrugAllergy class to a JSON object.
   * The JSON is expected to be valid against the NoKnownDrugAllergy JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/allergy/NoKnownDrugAllergy' };
