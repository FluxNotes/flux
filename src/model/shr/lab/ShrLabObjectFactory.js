...
/**
 * Generated object factory for the shr.lab namespace.
 */
export default class ShrLabObjectFactory {
  ...
  static createInstance(json, type) {
    ...
    if (namespace !== 'shr.lab') {
      throw new Error(`Unsupported type in ShrLabObjectFactory: ${type}`);
    }
    const klass = ClassRegistry.get('shr.lab', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrLabObjectFactory: ${type}`);
    }
  ...
  static createInstanceFromFHIR(shrType, fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    if (namespace !== 'shr.lab') {
      throw new Error(`Unsupported type in ShrLabObjectFactory: ${shrType}`);
    }
    const klass = ClassRegistry.get('shr.lab', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrLabObjectFactory: ${shrType}`);
