...
/**
 * Generated object factory for the shr.fhx namespace.
 */
export default class ShrFhxObjectFactory {
  /**
  ...
  static createInstance(json, type) {
    ...
    if (namespace !== 'shr.fhx') {
      throw new Error(`Unsupported type in ShrFhxObjectFactory: ${type}`);
    }
    const klass = ClassRegistry.get('shr.fhx', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrFhxObjectFactory: ${type}`);
    }
  ...
  static createInstanceFromFHIR(shrType, fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    if (namespace !== 'shr.fhx') {
      throw new Error(`Unsupported type in ShrFhxObjectFactory: ${shrType}`);
    }
    const klass = ClassRegistry.get('shr.fhx', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrFhxObjectFactory: ${shrType}`);
