...
import ClinicalStatement from '../core/ClinicalStatement';

/**
 * Generated class for shr.fhx.FamilyMemberHistory.
 * @extends ClinicalStatement
 */
class FamilyMemberHistory extends ClinicalStatement {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {FamilyMemberHistory} this.
   */
   ...
   * @returns {FamilyMember} The shr.fhx.FamilyMember
   */
  get familyMember() {
  ...
  /**
   * Set the FamilyMember.
   * This field/value is required.
   * @param {FamilyMember} familyMember - The shr.fhx.FamilyMember
   */
  set familyMember(familyMember) {
    this._familyMember = familyMember;
  }

  /**
   * Set the FamilyMember and return 'this' for chaining.
   * This field/value is required.
   * @param {FamilyMember} familyMember - The shr.fhx.FamilyMember
   * @returns {FamilyMemberHistory} this.
   */
  withFamilyMember(familyMember) {
    this.familyMember = familyMember; return this;
  }

  /**
   * Get the FamilyMemberCondition array.
   * @returns {Array<FamilyMemberCondition>} The shr.fhx.FamilyMemberCondition array
   */
  get familyMemberCondition() {
    return this._familyMemberCondition;
  }

  /**
   * Set the FamilyMemberCondition array.
   * @param {Array<FamilyMemberCondition>} familyMemberCondition - The shr.fhx.FamilyMemberCondition array
   */
  set familyMemberCondition(familyMemberCondition) {
    this._familyMemberCondition = familyMemberCondition;
  }

  /**
   * Set the FamilyMemberCondition array and return 'this' for chaining.
   * @param {Array<FamilyMemberCondition>} familyMemberCondition - The shr.fhx.FamilyMemberCondition array
   * @returns {FamilyMemberHistory} this.
   */
  withFamilyMemberCondition(familyMemberCondition) {
    this.familyMemberCondition = familyMemberCondition; return this;
  }

  /**
   * Deserializes JSON data to an instance of the FamilyMemberHistory class.
   * The JSON must be valid against the FamilyMemberHistory JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {FamilyMemberHistory} An instance of FamilyMemberHistory populated with the JSON data
   */
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('shr.fhx', 'FamilyMemberHistory');
  ...
  /**
   * Serializes an instance of the FamilyMemberHistory class to a JSON object.
   * The JSON is expected to be valid against the FamilyMemberHistory JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/shr/fhx/FamilyMemberHistory' };
    ...
    if (this.familyMember != null) {
      inst['FamilyMember'] = typeof this.familyMember.toJSON === 'function' ? this.familyMember.toJSON() : this.familyMember;
    }
    if (this.familyMemberCondition != null) {
      inst['FamilyMemberCondition'] = this.familyMemberCondition.map(f => f.toJSON());
    }
    return inst;
  }

  /**
   * Deserializes FHIR JSON data to an instance of the FamilyMemberHistory class.
   * The FHIR must be valid against the FamilyMemberHistory FHIR profile, although this is not validated by the function.
   ...
   * @returns {FamilyMemberHistory} An instance of FamilyMemberHistory populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/shr/fhx/FamilyMemberHistory', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      ...
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/shr-fhx-FamilyMember-extension') {
        inst.familyMember = FHIRHelper.createInstanceFromFHIR('shr.fhx.FamilyMember', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
      }
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/shr-fhx-FamilyMemberCondition-extension') {
        inst.familyMemberCondition = inst.familyMemberCondition || [];
        const inst_familyMemberCondition = FHIRHelper.createInstanceFromFHIR('shr.fhx.FamilyMemberCondition', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
        inst.familyMemberCondition.push(inst_familyMemberCondition);
      }
    }
    return inst;
  }
