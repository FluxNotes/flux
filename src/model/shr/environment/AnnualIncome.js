...
class AnnualIncome extends QuestionAnswer {
  ...
  /**
   * Serializes an instance of the AnnualIncome class to a JSON object.
   * The JSON is expected to be valid against the AnnualIncome JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/environment/AnnualIncome' };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the AnnualIncome class to a FHIR object.
   * The FHIR is expected to be valid against the AnnualIncome FHIR profile, but no validation checks are performed.
