...
/**
 * Generated class for shr.medication.MedicationAdherence.
 * @extends CodedNonLaboratoryObservation
 */
class MedicationAdherence extends CodedNonLaboratoryObservation {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {MedicationAdherence} this.
   */
  ...
  /**
   * Get the AdherenceFrequency.
   * @returns {AdherenceFrequency} The shr.medication.AdherenceFrequency
   */
  get adherenceFrequency() {
    return this._adherenceFrequency;
  }

  /**
   * Set the AdherenceFrequency.
   * This field/value is required.
   * @param {AdherenceFrequency} adherenceFrequency - The shr.medication.AdherenceFrequency
   */
  set adherenceFrequency(adherenceFrequency) {
    this._adherenceFrequency = adherenceFrequency;
  }

  /**
   * Set the AdherenceFrequency and return 'this' for chaining.
   * This field/value is required.
   * @param {AdherenceFrequency} adherenceFrequency - The shr.medication.AdherenceFrequency
   * @returns {MedicationAdherence} this.
   */
  withAdherenceFrequency(adherenceFrequency) {
    this.adherenceFrequency = adherenceFrequency; return this;
  }

  /**
   * Deserializes JSON data to an instance of the MedicationAdherence class.
   * The JSON must be valid against the MedicationAdherence JSON schema, although this is not validated by the function.
  ...
  /**
   * Serializes an instance of the MedicationAdherence class to a JSON object.
   * The JSON is expected to be valid against the MedicationAdherence JSON schema, but no validation checks are performed.
  ...
  toJSON() {
    ...
    if (this.adherenceFrequency != null) {
      inst['AdherenceFrequency'] = typeof this.adherenceFrequency.toJSON === 'function' ? this.adherenceFrequency.toJSON() : this.adherenceFrequency;
    }
    return inst;
  }

  /**
   * Serializes an instance of the MedicationAdherence class to a FHIR object.
   * The FHIR is expected to be valid against the MedicationAdherence FHIR profile, but no validation checks are performed.
  ...
  toFHIR(asExtension = false) {
    ...
    if (this.adherenceFrequency != null) {
      inst['extension'] = inst['extension'] || [];
      inst['extension'].push(typeof this.adherenceFrequency.toFHIR === 'function' ? this.adherenceFrequency.toFHIR(true) : this.adherenceFrequency);
    }
    if (this.findingStatus != null) {
  ...
  /**
   * Deserializes FHIR JSON data to an instance of the MedicationAdherence class.
   * The FHIR must be valid against the MedicationAdherence FHIR profile, although this is not validated by the function.
