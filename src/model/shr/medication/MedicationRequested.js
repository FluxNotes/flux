...
class MedicationRequested extends ActionRequested {
  ...
  static fromFHIR(fhir, asExtension=false) {
    ...
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-medication-SubstitutionAllowed-extension');
      if (match != null) {
        inst.substitutionAllowed = createInstanceFromFHIR('shr.medication.SubstitutionAllowed', match, true);
      }
    }
    if (fhir['status'] != null) {
    ...
    if (fhir['dispenseRequest'] != null && fhir['dispenseRequest']['performer'] != null) {
      ...
      inst.expectedPerformer.organization = createInstanceFromFHIR('shr.entity.Organization', fhir['dispenseRequest']['performer']);
    }
    return inst;
  }
