...
/**
 * Generated class for shr.behavior.ReligiousPracticeStatus.
 * @extends BehavioralFinding
 */
class ReligiousPracticeStatus extends BehavioralFinding {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {ReligiousPracticeStatus} this.
   */
  ...
  /**
   * Deserializes JSON data to an instance of the ReligiousPracticeStatus class.
   * The JSON must be valid against the ReligiousPracticeStatus JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {ReligiousPracticeStatus} An instance of ReligiousPracticeStatus populated with the JSON data
   */
  static fromJSON(json = {}) {
    const inst = new ReligiousPracticeStatus();
    setPropertiesFromJSON(inst, json);
    return inst;
  }
  /**
   * Serializes an instance of the ReligiousPracticeStatus class to a JSON object.
   * The JSON is expected to be valid against the ReligiousPracticeStatus JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/behavior/ReligiousPracticeStatus' };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the ReligiousPracticeStatus class to a FHIR object.
   * The FHIR is expected to be valid against the ReligiousPracticeStatus FHIR profile, but no validation checks are performed.
  ...
  toFHIR(asExtension = false) {
    ...
    if (this.reasonForBehavior != null) {
      inst['extension'] = inst['extension'] || [];
      inst['extension'].push(this.reasonForBehavior.toFHIR(true));
    }
    if (this.readinessToChange != null) {
      inst['extension'] = inst['extension'] || [];
      inst['extension'].push(this.readinessToChange.toFHIR(true));
    }
    if (this.outcome != null) {
      inst['extension'] = inst['extension'] || [];
...
export default ReligiousPracticeStatus;
