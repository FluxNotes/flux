...
/**
 * Generated class for shr.core.TimingOfDoses.
 */
class TimingOfDoses {
  ...
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('shr.core', 'TimingOfDoses');
   ...
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value' : 'http://standardhealthrecord.org/spec/shr/core/TimingOfDoses' } };
    if (this.value != null) {
  ...
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    const inst = new klass();
    if (asExtension) {
      inst.value = fhir['valueTiming'];
    }
    if (!asExtension && fhir != null) {
      inst.value = FHIRHelper.createInstanceFromFHIR('shr.core.Timing', fhir, fhirType, shrId, allEntries, mappedResources, referencesOut);
    }
    return inst;
  }
