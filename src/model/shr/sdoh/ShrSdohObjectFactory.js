...
/**
 * Generated object factory for the shr.sdoh namespace.
 */
export default class ShrSdohObjectFactory {
  /**
  ...
  static createInstance(json, type) {
    ...
    if (namespace !== 'shr.sdoh') {
      throw new Error(`Unsupported type in ShrSdohObjectFactory: ${type}`);
    }
    const klass = ClassRegistry.get('shr.sdoh', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrSdohObjectFactory: ${type}`);
    }
  ...
  static createInstanceFromFHIR(shrType, fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    if (namespace !== 'shr.sdoh') {
      throw new Error(`Unsupported type in ShrSdohObjectFactory: ${shrType}`);
    }
    const klass = ClassRegistry.get('shr.sdoh', elementName);
    if (!klass) {
      throw new Error(`Unsupported type in ShrSdohObjectFactory: ${shrType}`);
