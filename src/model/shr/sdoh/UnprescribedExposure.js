...
import NonLaboratoryObservation from '../core/NonLaboratoryObservation';

/**
 * Generated class for shr.sdoh.UnprescribedExposure.
 * @extends NonLaboratoryObservation
 */
class UnprescribedExposure extends NonLaboratoryObservation {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {UnprescribedExposure} this.
   */
  ...
  /**
   * Deserializes JSON data to an instance of the UnprescribedExposure class.
   * The JSON must be valid against the UnprescribedExposure JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {UnprescribedExposure} An instance of UnprescribedExposure populated with the JSON data
   */
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('shr.sdoh', 'UnprescribedExposure');
  ...
  /**
   * Serializes an instance of the UnprescribedExposure class to a JSON object.
   * The JSON is expected to be valid against the UnprescribedExposure JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/shr/sdoh/UnprescribedExposure' };
  ...
  /**
   * Deserializes FHIR JSON data to an instance of the UnprescribedExposure class.
   * The FHIR must be valid against the UnprescribedExposure FHIR profile, although this is not validated by the function.
   ...
   * @returns {UnprescribedExposure} An instance of UnprescribedExposure populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/shr/sdoh/UnprescribedExposure', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/shr-sdoh-SubstanceOrAgent-extension') {
...
export default UnprescribedExposure;
