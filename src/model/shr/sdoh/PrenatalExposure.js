...
/**
 * Generated class for shr.sdoh.PrenatalExposure.
 * @extends UnprescribedExposure
 */
class PrenatalExposure extends UnprescribedExposure {
  ...
  /**
   * Get the GestationalTimePeriod.
   * @returns {GestationalTimePeriod} The shr.core.GestationalTimePeriod
   */
  get gestationalTimePeriod() {
    return this._gestationalTimePeriod;
  }

  /**
   * Set the GestationalTimePeriod.
   * @param {GestationalTimePeriod} gestationalTimePeriod - The shr.core.GestationalTimePeriod
   */
  set gestationalTimePeriod(gestationalTimePeriod) {
    this._gestationalTimePeriod = gestationalTimePeriod;
  }

  /**
   * Set the GestationalTimePeriod and return 'this' for chaining.
   * @param {GestationalTimePeriod} gestationalTimePeriod - The shr.core.GestationalTimePeriod
   * @returns {PrenatalExposure} this.
   */
  withGestationalTimePeriod(gestationalTimePeriod) {
    this.gestationalTimePeriod = gestationalTimePeriod; return this;
  }
  ...
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('shr.sdoh', 'PrenatalExposure');
  ...
  /**
   * Serializes an instance of the PrenatalExposure class to a JSON object.
   * The JSON is expected to be valid against the PrenatalExposure JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/shr/sdoh/PrenatalExposure' };
    ...
    if (this.gestationalTimePeriod != null) {
      inst['GestationalTimePeriod'] = typeof this.gestationalTimePeriod.toJSON === 'function' ? this.gestationalTimePeriod.toJSON() : this.gestationalTimePeriod;
    }
    return inst;
  }

  /**
   * Deserializes FHIR JSON data to an instance of the PrenatalExposure class.
   * The FHIR must be valid against the PrenatalExposure FHIR profile, although this is not validated by the function.
   ...
   * @returns {PrenatalExposure} An instance of PrenatalExposure populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/shr/sdoh/PrenatalExposure', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      ...
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/shr-core-GestationalTimePeriod-extension') {
        inst.gestationalTimePeriod = FHIRHelper.createInstanceFromFHIR('shr.core.GestationalTimePeriod', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
      }
    }
