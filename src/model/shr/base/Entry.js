...
class Entry {
  ...
  static fromFHIR(fhir, asExtension=false) {
    ...
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-core-CreationTime-extension');
      if (match != null) {
        inst.creationTime = createInstanceFromFHIR('shr.core.CreationTime', match, true);
      }
    }
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-base-RecordStatus-extension');
      if (match != null) {
        inst.recordStatus = createInstanceFromFHIR('shr.base.RecordStatus', match, true);
      }
    }
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-base-DerivedFrom-extension');
      if (match != null) {
        inst.derivedFrom = createInstanceFromFHIR('shr.base.DerivedFrom', match, true);
      }
    }
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-base-RecordedBy-extension');
      if (match != null) {
        inst.recordedBy = createInstanceFromFHIR('shr.base.RecordedBy', match, true);
      }
    }
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-base-SignedBy-extension');
      if (match != null) {
        inst.signedBy = createInstanceFromFHIR('shr.base.SignedBy', match, true);
      }
    }
    return inst;
  }
