...
class Travel extends QuestionAnswer {
  ...
  /**
   * Serializes an instance of the Travel class to a JSON object.
   * The JSON is expected to be valid against the Travel JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value': 'http://standardhealthrecord.org/spec/shr/lifehistory/Travel' };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the Travel class to a FHIR object.
   * The FHIR is expected to be valid against the Travel FHIR profile, but no validation checks are performed.
  ...
  toFHIR(asExtension = false) {
    ...
    if (this.value != null) {
      inst['valueString'] = typeof this.value.toFHIR === 'function' ? this.value.toFHIR() : this.value;
    }
    if (this.valueAbsentReason != null) {
