...
class MilitaryBranch extends QuestionAnswer {
  ...
  /**
   * Serializes an instance of the MilitaryBranch class to a JSON object.
   * The JSON is expected to be valid against the MilitaryBranch JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value': 'http://standardhealthrecord.org/spec/shr/lifehistory/MilitaryBranch' } };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the MilitaryBranch class to a FHIR object.
   * The FHIR is expected to be valid against the MilitaryBranch FHIR profile, but no validation checks are performed.
