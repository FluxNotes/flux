...
class PatientInstructions {
   ...
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value': 'http://standardhealthrecord.org/spec/shr/action/PatientInstructions' } };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the PatientInstructions class to a FHIR object.
   * The FHIR is expected to be valid against the PatientInstructions FHIR profile, but no validation checks are performed.
  ...
  toFHIR(asExtension = false) {
    let inst = {};
    if (asExtension) {
      inst['url'] = 'http://standardhealthrecord.org/fhir/StructureDefinition/shr-action-PatientInstructions-extension';
