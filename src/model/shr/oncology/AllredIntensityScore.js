...
class AllredIntensityScore extends ObservationComponent {
   ...
   * This field/value is required.
   * @param {Quantity} value - The shr.core.Quantity
   * @returns {AllredIntensityScore} this.
   */
  withValue(value) {
  ...
  /**
   * Serializes an instance of the AllredIntensityScore class to a JSON object.
   * The JSON is expected to be valid against the AllredIntensityScore JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value': 'http://standardhealthrecord.org/spec/shr/oncology/AllredIntensityScore' } };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the AllredIntensityScore class to a FHIR object.
   * The FHIR is expected to be valid against the AllredIntensityScore FHIR profile, but no validation checks are performed.
