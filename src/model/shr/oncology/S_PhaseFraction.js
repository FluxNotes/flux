...
class S_PhaseFraction extends Observation {
  ...
  /**
   * Serializes an instance of the S_PhaseFraction class to a JSON object.
   * The JSON is expected to be valid against the S_PhaseFraction JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value': 'http://standardhealthrecord.org/spec/shr/oncology/S_PhaseFraction' } };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the S_PhaseFraction class to a FHIR object.
   * The FHIR is expected to be valid against the S_PhaseFraction FHIR profile, but no validation checks are performed.
