...
class Refseq extends ObservationComponent {
  ...
  /**
   * Serializes an instance of the Refseq class to a JSON object.
   * The JSON is expected to be valid against the Refseq JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = { 'EntryType': { 'Value': 'http://standardhealthrecord.org/spec/shr/oncology/Refseq' } };
    if (this.value != null) {
  ...
  /**
   * Serializes an instance of the Refseq class to a FHIR object.
   * The FHIR is expected to be valid against the Refseq FHIR profile, but no validation checks are performed.
