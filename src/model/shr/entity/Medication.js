...
class Medication extends Entity {
  ...
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('shr.entity', 'Medication');
  ...
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/shr/entity/Medication', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_extension of fhir['extension'] || []) {
      ...
      if (fhir_extension['url'] != null && fhir_extension['url'] === 'http://example.com/fhir/StructureDefinition/shr-entity-OverTheCounter-extension') {
        inst.overTheCounter = FHIRHelper.createInstanceFromFHIR('shr.entity.OverTheCounter', fhir_extension, 'Extension', shrId, allEntries, mappedResources, referencesOut, true);
      }
    }
    if (fhir['code'] != null) {
    ...
    if (fhir['isBrand'] != null) {
      inst.brand = inst.brand || FHIRHelper.createInstanceFromFHIR('shr.entity.Brand', {}, null, shrId);
      inst.brand.isBrand = FHIRHelper.createInstanceFromFHIR('shr.entity.IsBrand', fhir['isBrand'], 'boolean', shrId, allEntries, mappedResources, referencesOut, false);
    }
    ...
    if (fhir['product'] != null) {
      if (fhir['product']['form'] != null) {
        inst.doseForm = FHIRHelper.createInstanceFromFHIR('shr.entity.DoseForm', fhir['product']['form'], 'CodeableConcept', shrId, allEntries, mappedResources, referencesOut, false);
      }
      if (fhir['product']['ingredient'] != null && fhir['product']['ingredient'][0] != null) {
        if (fhir['product']['ingredient'][0]['item'] != null) {
          ...
          const inst_ingredient = FHIRHelper.createInstanceFromFHIR('shr.entity.Ingredient', {}, null, shrId);
          inst.ingredient.push(inst_ingredient);
          const entryId = fhir['product']['ingredient'][0]['item']['reference'];
          if (!mappedResources[entryId]) {
            const referencedEntry = allEntries.find(e => e.fullUrl === entryId);
            if (referencedEntry) {
              mappedResources[entryId] = FHIRHelper.createInstanceFromFHIR('shr.entity.Substance', referencedEntry['resource'], 'undefined', shrId, allEntries, mappedResources, referencesOut);
            }
          }
        ...
        if (fhir['product']['ingredient'][0]['amount'] != null) {
          ...
          const inst_ingredient = FHIRHelper.createInstanceFromFHIR('shr.entity.Ingredient', {}, null, shrId);
          inst.ingredient.push(inst_ingredient);
          inst_ingredient.ingredientAmount = FHIRHelper.createInstanceFromFHIR('shr.entity.IngredientAmount', fhir['product']['ingredient'][0]['amount'], 'Ratio', shrId, allEntries, mappedResources, referencesOut, false);
        }
      }
      if (fhir['product']['batch'] != null && fhir['product']['batch'][0] != null) {
        if (fhir['product']['batch'][0]['lotNumber'] != null) {
          inst.lotNumber = FHIRHelper.createInstanceFromFHIR('shr.entity.LotNumber', fhir['product']['batch'][0]['lotNumber'], 'string', shrId, allEntries, mappedResources, referencesOut, false);
        }
        if (fhir['product']['batch'][0]['expirationDate'] != null) {
          inst.expirationDate = FHIRHelper.createInstanceFromFHIR('shr.entity.ExpirationDate', fhir['product']['batch'][0]['expirationDate'], 'dateTime', shrId, allEntries, mappedResources, referencesOut, false);
        }
      }
    }
    if (fhir['package'] != null) {
      if (fhir['package']['container'] != null) {
        inst.package = FHIRHelper.createInstanceFromFHIR('shr.entity.Package', fhir['package']['container'], 'CodeableConcept', shrId, allEntries, mappedResources, referencesOut, false);
      }
    }
    if (asExtension) {
