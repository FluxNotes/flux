...
class SpecimenCollectionPerformed extends ProcedurePerformed {
  ...
  toFHIR(asExtension=false) {
    ...
    if (this.amountOrSize != null) {
      inst['extension'] = inst['extension'] || [];
      inst['extension'].push(typeof this.amountOrSize.toFHIR === 'function' ? this.amountOrSize.toFHIR(true) : this.amountOrSize);
    }
  ...
  static fromFHIR(fhir, asExtension=false) {
    ...
    if (fhir['extension'] != null) {
      const match = fhir['extension'].find(e => e.url == 'http://example.com/fhir/StructureDefinition/shr-procedure-AmountOrSize-extension');
      if (match != null) {
        inst.amountOrSize = createInstanceFromFHIR('shr.procedure.AmountOrSize', match, true);
      }
    }
