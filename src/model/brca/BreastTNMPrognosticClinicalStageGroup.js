...
/**
 * Generated class for brca.BreastTNMPrognosticClinicalStageGroup.
 * @extends TNMClinicalStageGroup
 */
class BreastTNMPrognosticClinicalStageGroup extends TNMClinicalStageGroup {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {BreastTNMPrognosticClinicalStageGroup} this.
   */
  ...
  /**
   * Deserializes JSON data to an instance of the BreastTNMPrognosticClinicalStageGroup class.
   * The JSON must be valid against the BreastTNMPrognosticClinicalStageGroup JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {BreastTNMPrognosticClinicalStageGroup} An instance of BreastTNMPrognosticClinicalStageGroup populated with the JSON data
   */
  static fromJSON(json={}) {
    const klass = ClassRegistry.get('brca', 'BreastTNMPrognosticClinicalStageGroup');
  ...
  /**
   * Serializes an instance of the BreastTNMPrognosticClinicalStageGroup class to a JSON object.
   * The JSON is expected to be valid against the BreastTNMPrognosticClinicalStageGroup JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/brca/BreastTNMPrognosticClinicalStageGroup' };
  ...
  /**
   * Deserializes FHIR JSON data to an instance of the BreastTNMPrognosticClinicalStageGroup class.
   * The FHIR must be valid against the BreastTNMPrognosticClinicalStageGroup FHIR profile, although this is not validated by the function.
   ...
   * @returns {BreastTNMPrognosticClinicalStageGroup} An instance of BreastTNMPrognosticClinicalStageGroup populated with the FHIR data
   */
  static fromFHIR(fhir, fhirType, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/brca/BreastTNMPrognosticClinicalStageGroup', 'uri');
    if (fhir['meta'] != null) {
    ...
    for (const fhir_related of fhir['related'] || []) {
      ...
      if (fhir_related['target'] != null && fhir_related['target']['reference'] != null && FHIRHelper.conformsToProfile(allEntries.find(e => e.fullUrl === fhir_related['target']['reference']), 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/onco-core-TNMClinicalPrimaryTumorCategory')) {
      ...
      if (fhir_related['target'] != null && fhir_related['target']['reference'] != null && FHIRHelper.conformsToProfile(allEntries.find(e => e.fullUrl === fhir_related['target']['reference']), 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/onco-core-TNMClinicalRegionalNodesCategory')) {
        if (fhir_related['target'] != null) {
          inst.panelMembers.observation = inst.panelMembers.observation || [];
          const entryId = fhir_related['target']['reference'];
          if (!mappedResources[entryId]) {
      ...
      if (fhir_related['target'] != null && fhir_related['target']['reference'] != null && FHIRHelper.conformsToProfile(allEntries.find(e => e.fullUrl === fhir_related['target']['reference']), 'http://hl7.org/fhir/us/shr/DSTU2/StructureDefinition/onco-core-TNMClinicalDistantMetastasesCategory')) {
        if (fhir_related['target'] != null) {
          inst.panelMembers.observation = inst.panelMembers.observation || [];
          const entryId = fhir_related['target']['reference'];
          if (!mappedResources[entryId]) {
...
export default BreastTNMPrognosticClinicalStageGroup;
