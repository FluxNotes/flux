...
/**
 * Generated class for brca.OncotypeDxInvasiveRecurrenceScore.
 * @extends SimpleLaboratoryObservation
 */
class OncotypeDxInvasiveRecurrenceScore extends SimpleLaboratoryObservation {
   ...
   * @param {Entry} entryInfo - The shr.base.Entry
   * @returns {OncotypeDxInvasiveRecurrenceScore} this.
   */
  ...
  /**
   * Deserializes JSON data to an instance of the OncotypeDxInvasiveRecurrenceScore class.
   * The JSON must be valid against the OncotypeDxInvasiveRecurrenceScore JSON schema, although this is not validated by the function.
   * @param {object} json - the JSON data to deserialize
   * @returns {OncotypeDxInvasiveRecurrenceScore} An instance of OncotypeDxInvasiveRecurrenceScore populated with the JSON data
   */
  static fromJSON(json={}) {
    const inst = new OncotypeDxInvasiveRecurrenceScore();
    setPropertiesFromJSON(inst, json);
    return inst;
  }

  /**
   * Serializes an instance of the OncotypeDxInvasiveRecurrenceScore class to a JSON object.
   * The JSON is expected to be valid against the OncotypeDxInvasiveRecurrenceScore JSON schema, but no validation checks are performed.
   * @returns {object} a JSON object populated with the data from the element
   */
  toJSON() {
    const inst = this._entryInfo.toJSON();
    inst['EntryType'] = { 'Value' : 'http://standardhealthrecord.org/spec/brca/OncotypeDxInvasiveRecurrenceScore' };
  ...
  /**
   * Deserializes FHIR JSON data to an instance of the OncotypeDxInvasiveRecurrenceScore class.
   * The FHIR must be valid against the OncotypeDxInvasiveRecurrenceScore FHIR profile, although this is not validated by the function.
   ...
   * @returns {OncotypeDxInvasiveRecurrenceScore} An instance of OncotypeDxInvasiveRecurrenceScore populated with the FHIR data
   */
  static fromFHIR(fhir, shrId=uuid(), allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
    ...
    inst.entryInfo.entryType = FHIRHelper.createInstanceFromFHIR('shr.base.EntryType', 'http://standardhealthrecord.org/spec/brca/OncotypeDxInvasiveRecurrenceScore');
    if (fhir['meta'] != null) {
    ...
    if (fhir['valuePeriod'] != null) {
      inst.findingResult = FHIRHelper.createInstanceFromFHIR('shr.base.FindingResult', fhir['valuePeriod'], shrId, allEntries, mappedResources, referencesOut, false);
    }
    ...
    if (fhir['referenceRange'] != null && fhir['referenceRange'][0] != null) {
      ...
      if (fhir['referenceRange'][0]['type'] != null) {
        inst.referenceRange.type = FHIRHelper.createInstanceFromFHIR('shr.core.Type', fhir['referenceRange'][0]['type'], shrId, allEntries, mappedResources, referencesOut, false);
      }
      if (fhir['referenceRange'][0]['appliesTo'] != null && fhir['referenceRange'][0]['appliesTo'][0] != null) {
        inst.referenceRange.applicableSubpopulation = FHIRHelper.createInstanceFromFHIR('shr.base.ApplicableSubpopulation', fhir['referenceRange'][0]['appliesTo'][0], shrId, allEntries, mappedResources, referencesOut, false);
      }
      if (fhir['referenceRange'][0]['age'] != null) {
        inst.referenceRange.applicableAgeRange = FHIRHelper.createInstanceFromFHIR('shr.base.ApplicableAgeRange', fhir['referenceRange'][0]['age'], shrId, allEntries, mappedResources, referencesOut, false);
      }
    }
...
export default OncotypeDxInvasiveRecurrenceScore;
