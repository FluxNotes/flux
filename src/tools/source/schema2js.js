...
Generator.prototype = {
	...
	code: function () {
		...
		for (var url in this.aliases) {
			...
			if (this.config.classes === false) {
				...
				code += propertyExpression('classes', urlName) + '.prototype.constructor = ' + propertyExpression('classes', urlName) + ';\n';
			}
	...
	codeForUrl: function (url, requireUrl) {
		...
		if (!this.schemaAcceptsType(schema, 'object') || this.config.classes === false) {
			...
			// doesnt even find Progression - class
			//if(classExpression === "OncologyDefinitionsProgressionClass" || classExpression === "BaseDefinitionsActionClass"){
				
			//	console.log("TORCH: " + classExpression);
			//	console.log(schema.properties);
				
			//	console.log("EndTorch");
			//}
			// Defaults and property conversion
			for (var key in schema.properties || {}) {
				// wherever we use key, need some illegal char fixing copy-pasted. if - or / make it _
				// problematic schema items either have one - or two /
				var editedKey = key;
				var illegalMinus = key.indexOf('-');
				if(illegalMinus > -1){
					editedKey = key.substr(0, illegalMinus) + '_' + key.substr(illegalMinus+1);
				}
				var illegalSlash = key.indexOf('/');
				if(illegalSlash > -1){
					editedKey = key.substr(0, illegalSlash) + '_' + key.substr(illegalSlash+1);
					var illegalSlash = editedKey.indexOf('/');
					if(illegalSlash > -1){
						editedKey = editedKey.substr(0, illegalSlash) + '_' + editedKey.substr(illegalSlash+1);
					}
				}
				// console.log("OriginalLoop: key was " + key + " and editedKey is " + editedKey);
				// this is convered successfully
				var subSchema = this.getFullSchema(schema.properties[key]);
				body += 'if (typeof ' + propertyExpression('value', editedKey) + ' !== "undefined") {\n';
				body += indent(propertyExpression('this', editedKey) + ' = ' + propertyExpression('value', editedKey) + ';\n');
				if ('default' in subSchema) {
					body += '} else {\n';
					body += indent(propertyExpression('this', editedKey) + ' = ' + JSON.stringify(subSchema['default']) + ';\n');
				}
				body += '}\n';
				var subUrl = (subSchema && subSchema.id) || this.extendUrl(url, ['properties', editedKey]);
				castProperty(subSchema, subUrl, propertyExpression('this', editedKey));
			}
			// Defaults and property conversion - Torch added for e.g. Progression - level deeper
			var memberIndex = 0;
			if(schema.allOf){
				//console.log("Not Null allOf "+ classExpression);
				for(var item in schema.allOf){
			...
			//	console.log(schema.allOf[memberIndex].properties); // schema.allOf[memberIndex].properties for things hidden in allOf
			}
				
			if(schema.allOf && schema.allOf[memberIndex]) {				
				for (var key in schema.allOf[memberIndex].properties || {}) {
					// wherever we use key, need some illegal char fixing copy-pasted. if - or / make it _
					// problematic schema items either have one - or two /
					var editedKey = key;
					var illegalMinus = key.indexOf('-');
					...
					var subSchema = this.getFullSchema(schema.allOf[memberIndex].properties[key]); //Leave as 'key', is an index into this map
					body += 'if (typeof ' + propertyExpression('value', editedKey) + ' !== "undefined") {\n';
					body += indent(propertyExpression('this', editedKey) + ' = ' + propertyExpression('value', editedKey) + ';\n');
					if ('default' in subSchema) {
						body += '} else {\n';
					...
					body += '}\n';
					var subUrl = (subSchema && subSchema.id) || this.extendUrl(url, ['properties', editedKey]); //TODO: change these to editedKey too?
					castProperty(subSchema, subUrl, propertyExpression('this', editedKey));
				} 
			}//end Torch added
			...
			//TODO: try to make this the actual name of the super class. (with or without "Class" ending). 
			// schema2js originally assumed one level of objects, didn't have allOf support. Every object inherited from superclass.
			if(schema.allOf && memberIndex > 0){
				var fullURL = schema.allOf[memberIndex-1]['$ref']; // Gets URL
				// cut out the http://jstars-linux-1.mitre.org/json-schema/shr/
				// TODO: make flexible on hostname, could cut it at the 5th forward slash.
				var partialURL = fullURL.substr(48); // should get us e.g. assessment#/definitions/Assessment
				var indexOfHashSlash = partialURL.indexOf("#/");
				var indexOfSecondSlash = partialURL.indexOf("/", indexOfHashSlash+2);
				//toUpper twice and remove non-alpha chars, add Class to the end to make it a class name.
				superclassExpr = partialURL.charAt(0).toUpperCase() + partialURL.substr(1,indexOfHashSlash-1) + partialURL.charAt(indexOfHashSlash+2).toUpperCase() +
					partialURL.substr(indexOfHashSlash+3, indexOfSecondSlash-indexOfHashSlash-3) + partialURL.substr(indexOfSecondSlash+1) + "Class"; 
				//console.log("Superclass is " + superclassExpr + " generated from " + partialURL); 
				/*console.log("Start of strings for: " + partialURL);
				console.log(partialURL.charAt(0).toUpperCase());
				console.log(partialURL.substr(1,indexOfHashSlash-1));
				console.log(partialURL.charAt(indexOfHashSlash+2).toUpperCase());
				console.log(partialURL.substr(indexOfHashSlash+3, indexOfSecondSlash-indexOfHashSlash-3));
				console.log(partialURL.substr(indexOfSecondSlash+1));*/
			...
			code += classExpression + '.prototype.constructor = ' + classExpression + ';\n';
			if (schema.title) {
		...
		if (this.config.validation) { // setting this to false removes 99% of the generated code, doesn't traverse type tree
			...
			code += indent(this.validationCode('value', [], url, schema, requireUrl, function (errorExpr, single) {
				//console.log("Verified Last minus sign when Ki-67L - buried in param: " + errorExpr );
		...
		// Adds property definitions on each object definition
		code += '\n';
		for (var key in schema.properties || {}) {
				// wherever we use key, need some illegal char fixing copy-pasted. if - or / make it _
				// problematic schema items either have one - or two /
			...
			code += 'Object.defineProperty(' + classExpression + ', ' + propertyExpression(classExpression, editedKey) + ', {\n';
			code += indent('get: function() { return ' + propertyExpression(classExpression, editedKey) + '; },\n');
			code += indent('set: function(newValue) { ' + propertyExpression(classExpression, editedKey) + ' = newValue; },\n');
			code += indent('enumerable: true,\n');
			code += indent('configurable: true\n');
			code += '});\n';
		}
		// subclass types need properties too, they are under allOf
		if(schema.allOf && schema.allOf[memberIndex]) {				
			for (var key in schema.allOf[memberIndex].properties || {}) {
				// wherever we use key, need some illegal char fixing copy-pasted. if - or / make it _
				// problematic schema items either have one - or two /
				...
				//console.log("key was " + key + " and editedKey is " + editedKey);
				code += 'Object.defineProperty(' + classExpression + ', ' + propertyExpression(classExpression, editedKey) + ', {\n';
				code += indent('get: function() { return ' + propertyExpression(classExpression, editedKey) + '; },\n');
				code += indent('set: function(newValue) { ' + propertyExpression(classExpression, editedKey) + ' = newValue; },\n');
				code += indent('enumerable: true,\n');
	...
	validationCode: function (valueExpr, dataPathExprs, schemaUrl, schema, requireUrl, errorFunc, useReference) {
		...
		if (!allowedType('object')) {
			...
			if (schema.patternProperties || 'additionalProperties' in schema) {
				...
				for (var key in schema.patternProperties) { // fix /,- here too? torch
