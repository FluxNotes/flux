...
class ScatterPlotVisualizer extends Visualizer {
    ...
    buildChart = (parsedData, conditionSection) => {
        this.setState({
            // We've gotten this far in our promise -- time to cancel loading animation
            loading: false,
        });
    ...
    renderScatterPlot = (patient, condition, conditionSection) => {
        ...
        if (Lang.isString(myData) || (Lang.isObject(myData) && Lang.isUndefined(myData.then))) {
            ...
            // timeout needed for css transition when opening/closing notes
            setTimeout(function() { this.setState({ chartHeight: '600px' }, () => { this.buildChart(myData, conditionSection); }); }.bind(this), 500);
            return;
        } else {
            this.setState({
            ...
            if (Lang.isObject(myData) && !Lang.isUndefined(myData.then)) {
                // myData should be a promise initially
                myData.then(parsedData => {
                    this.buildChart(parsedData, conditionSection);
                }).catch(err => {
